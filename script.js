...
let html5QrCode = null; // Declarado aquí para que sea accesible en todo el ámbito

// --- Nueva función para cargar Html5Qrcode si no está definido ---
// Esta función se encarga de asegurar que la librería esté disponible.
async function ensureHtml5QrcodeLoaded() {
    return new Promise((resolve, reject) => {
        // console.log("Verificando Html5Qrcode...");
        ...
        script.onload = () => {
            if (typeof Html5Qrcode !== 'undefined') {
                ...
                reject(new Error("Html5Qrcode no se definió después de la carga dinámica. Es posible que haya un problema con la URL de la librería o un conflicto."));
            }
        };
        script.onerror = (e) => reject(new Error("Error al cargar html5-qrcode.min.js dinámicamente. Verifique la URL o su conexión a internet."));
        document.head.appendChild(script); // Añadir al head para que se ejecute y defina globalmente
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    adminScanQRBtn = document.getElementById('admin-scan-qr-btn'); // Esta es la línea 68 aproximadamente
    ...
    // let html5QrCode = null; // Ya declarado arriba en el ámbito global

    if (adminScanQRBtn) { // <--- This is the line that caused the ReferenceError if adminScanQRBtn was not initialized yet.
        adminScanQRBtn.addEventListener('click', async () => {
            try {
